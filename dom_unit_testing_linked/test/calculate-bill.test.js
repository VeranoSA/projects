describe('Calculate Bill', function(){
    ...
    it("should return class name 'warning' if Total cost exceeds 20", function(){
        let calcuBill = calculateBill();
            var okay = calcuBill.billCalculate("call, sms, call, sms, sms, call, sms, call, sms, sms, sms, call, sms, call, sms,");
        
        assert.equal('warning', calcuBill.totalClassName(okay));
    });
    it("should return class name 'danger' if Total cost exceeds 30", function(){
        let calcuBill = calculateBill();
           var okay = calcuBill.billCalculate("call, call, call, sms, sms, call, sms, call, sms, sms, sms, call, call, call, sms, sms, sms, sms, call, sms, sms, sms, sms, call, sms,");
        
        assert.equal('danger', calcuBill.totalClassName(okay));
    });
});